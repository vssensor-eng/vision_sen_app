...
checks={
 ...
 'version': 'APP_VERSION = "0.6.9"' in s,
 ...
 'log_always_visible': 'TEKNİK LOG' in s and 'Logu Göster' not in s and 'Logu Gizle' not in s,
 'log_toolbar': all(x in s for x in ('Temizle','Kopyala','Kaydet','Otomatik Kaydır','def copy_log','def save_log')),
 'log_independent_scroll': 'orient="vertical", command=self.log_text.yview' in s and 'orient="horizontal", command=self.log_text.xview' in s,
 'left_scroll_independent': 'def make_scroll_tab()' in s and 'canvas.yview' in s,
 'right_resizable_log': 'self.main_pane = tk.PanedWindow(root, orient="horizontal"' in s and 'self.main_pane.add(log_wrap, minsize=390' in s and '_place_initial_log_sash' in s,
...
if failed:
    raise SystemExit('FACTORY 0.6.9 PRODUCTION GATE FAIL: '+', '.join(failed))
print('FACTORY 0.6.9 PRODUCTION GATE PASS')
