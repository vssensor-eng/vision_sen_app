...
class _VerifyScreenState extends State<VerifyScreen> {
  ...
  Future<void> _run() async {
    ...
    if (!connected) {
      setState(() {
        failed = true;
        errorText = 'Cihazla Bluetooth bağlantısı veya Android eşleştirmesi '
            'tamamlanamadı.\n\nİlk bağlantıda telefonun gösterdiği eşleştirme '
            'isteğini onaylayın. Cihazın 60 saniyelik Bluetooth kurulum penceresi '
            'kapanmışsa cihazı yeniden açıp tekrar deneyin.';
      });
      return;
    }
    setState(() => step = 1);
    ...
    if (deviceInfo == null) {
      setState(() {
        failed = true;
        errorText = 'Cihaz bilgileri güvenli BLE bağlantısından okunamadı.\n\n'
            'Eşleştirme onaylandıysa bağlantıyı yeniden deneyin.';
      });
    ...
    if (deviceInfo['protocol_mismatch'] == true) {
      setState(() {
        failed = true;
        errorText = 'Cihazın reklam paketindeki BLE protokolü ile INFO verisi '
            'eşleşmiyor. Güvenli kurulum için işlem durduruldu.';
      });
      await widget.ble.disconnect();
      return;
    }
    if (deviceInfo['identity_fields_valid'] != true ||
        protocol is! int ||
        deviceType.isEmpty) {
      setState(() {
        failed = true;
        errorText = 'Firmware zorunlu VisionSen kimlik alanlarını göndermiyor. '
            'Bu uygulama yalnızca device_type ve protocol_version alanlarını '
            'açıkça bildiren firmware ile çalışır.';
      });
      await widget.ble.disconnect();
      return;
    }
    if (!VisionSenCompatibility.isSupportedIdentity(deviceType, protocol)) {
      setState(() {
        ...
        errorText = 'Bu firmware kimliği desteklenmiyor: '
            '${VisionSenCompatibility.deviceTypeLabel(deviceType)} / '
            'BLE protokol v$protocol.';
      });
      await widget.ble.disconnect();
      return;
    }
    ...
    if (serial.isNotEmpty &&
        ...
        (configured || widget.config.serial.isEmpty)) {
  ...
  Widget build(BuildContext c) {
    final labels = [
      'Cihaza bağlanılıyor / eşleştiriliyor',
      'Cihaz bilgileri okunuyor',
      'Cihaz uyumluluğu doğrulanıyor',
    ];
    final done = step >= 3;
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                child: Icon(
                  failed ? Icons.error_outline : Icons.security,
                  size: 60,
