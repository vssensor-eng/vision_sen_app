...
class MobileApiService {
  ...
  Map<String, String> _headers({bool json = false}) {
    ...
    if (currentToken != null && currentToken.isNotEmpty) {
      ...
      headers['X-OIM-Mobile-Token'] = currentToken;
    }
    return headers;
  }
  ...
  Future<Map<String, dynamic>> _write(
    ...
    if (method == 'POST') {
      ...
      throw const MobileApiException('Desteklenmeyen mobil API işlemi.');
    }
  ...
  Future<Map<String, dynamic>> login({
    ...
    return _write('POST', '/mobile/login', {
      'login': login,
      'password': password,
      'remember': remember,
    });
  ...
  Future<Map<String, dynamic>> bundle() async {
    final response = await _client
        .get(_uri('/mobile/bundle'), headers: _headers())
        .timeout(const Duration(seconds: 20));
    return _decode(response);
  }
  ...
  Future<Map<String, dynamic>> deleteDevice(int id) {
    return _write('DELETE', '/mobile/devices/$id', const {});
  }
  ...
  Future<Map<String, dynamic>> deleteLocation(int id) {
    return _write('DELETE', '/mobile/locations/$id', const {});
  }
  ...
  Future<Map<String, dynamic>> createSensor({
    ...
    return _write('POST', '/mobile/sensors', {
      'device_id': deviceId,
      'metric': metric,
      'enabled': enabled ? 1 : 0,
      'min_value': minValue,
  ...
  Future<Map<String, dynamic>> updateSensor(
    ...
    return _write('PUT', '/mobile/sensors/$id', {
      'device_id': deviceId,
      'metric': metric,
      'enabled': enabled ? 1 : 0,
      'min_value': minValue,
  ...
  Future<Map<String, dynamic>> deleteSensor(int id) {
    return _write('DELETE', '/mobile/sensors/$id', const {});
  }

  Future<void> logout() async {
    try {
      await _write('POST', '/mobile/logout', const {});
    } catch (_) {}
  }
